...
/** Computes heap-usage statistics from parsed GC events. */
public interface HeapAnalyzer {

  /**
   * Analyzes heap usage from parsed events.
   *
   * @param events parsed GC events
   * @return aggregate heap statistics
   */
  HeapStats analyze(List<GCEvent> events);
}
