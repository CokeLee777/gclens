...
/** Parses a single G1 young GC log line. */
public class G1YoungGCLineParser implements GCLogLineParser {

  private static final Pattern PATTERN =
      Pattern.compile("\\[([\\d.]+)s].*Pause Young.*?(\\d+)M->(\\d+)M\\((\\d+)M\\)\\s+([\\d.]+)ms");

  /** Creates a new parser. */
  public G1YoungGCLineParser() {}

  /**
   * Attempts to parse one log line into a {@link io.github.cokelee777.gclens.g1gc.model.G1YoungGC}
   * event.
   *
  ...
  public ParseResult parse(String line) {
    ...
    if (line.isBlank() || !line.contains("Pause Young")) {
