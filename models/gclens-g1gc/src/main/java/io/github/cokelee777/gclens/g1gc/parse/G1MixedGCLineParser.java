...
/** Parses a single G1 mixed GC log line. */
public class G1MixedGCLineParser implements GCLogLineParser {

  private static final Pattern PATTERN =
      Pattern.compile("\\[([\\d.]+)s].*Pause Mixed.*?(\\d+)M->(\\d+)M\\((\\d+)M\\)\\s+([\\d.]+)ms");

  /** Creates a new parser. */
  public G1MixedGCLineParser() {}

  /**
   * Attempts to parse one log line into a {@link io.github.cokelee777.gclens.g1gc.model.G1MixedGC}
   * event.
   *
  ...
  public ParseResult parse(String line) {
    ...
    if (line.isBlank() || !line.contains("Pause Mixed")) {
